...
import type { Task, TaskAttachment, Profile } from '@/types/database'
import {
  X,
  Calendar,
  Clock,
...
interface NativeTaskDetailModalProps {
  task: Task | null
  visible: boolean
  onClose: () => void
...
export function NativeTaskDetailModal({
  task,
  visible,
  onClose,
  currentUserId,
  ...
  const canEditOrDelete = isOwner || (!task.is_private && isAdmin)

  const formatDueDateFull = (dateStr?: string) => {
    ...
    try {
      ...
      if (isCompleted) {
        return {
          text: isToday ? `Entregado hoy a las ${timeStr}` : `Entregado • ${dayName} ${dayNum} de ${monthName}, ${timeStr}`,
          isOverdue: false,
          isToday: false,
        }
      }
      ...
      if (isPast) {
        return {
          text: isToday ? `Venció hoy a las ${timeStr}` : `Venció el ${dayName} ${dayNum} de ${monthName}, ${timeStr}`,
          isOverdue: true,
          isToday,
        }
      }
      ...
      return {
        text: `Vence el ${dayName} ${dayNum} de ${monthName}, ${timeStr}`,
        isOverdue: false,
        isToday: false,
      }
    } catch {
  ...
  const dueInfo = formatDueDateFull(task.due_date)
  ...
  const handleDelete = () => {
    Alert.alert(
      '¿Eliminar esta tarea?',
      'Esta acción no se puede deshacer. Se eliminarán sus notas y archivos adjuntos.',
      [
    ...
    >
      <View style={styles.backdrop}>
        ...
        <View style={styles.sheetContainer}>
          {/* Header con Handle de arrastre y Cerrar */}
          ...
          <ScrollView style={styles.sheetScroll} showsVerticalScrollIndicator={false}>
            {/* Badges de Materia, Tipo y Estado */}
            <View style={styles.badgesRow}>
              ...
              {task.is_private ? (
                <View style={styles.privatePill}>
                  ...
                  <Text style={styles.privatePillText}>Privada</Text>
                </View>
              ) : (
                <View style={styles.classroomPill}>
                  <Users size={10} color="#818CF8" />
                  <Text style={styles.classroomPillText}>Salón</Text>
                </View>
              )}
            ...
            {task.description ? (
              <View style={styles.section}>
                <Text style={styles.sectionLabel}>INSTRUCCIONES / NOTAS</Text>
            ...
            {attachments.length > 0 && (
              <View style={styles.section}>
                <Text style={styles.sectionLabel}>ARCHIVOS Y ENLACES ADJUNTOS ({attachments.length})</Text>
                <View style={styles.attachmentsList}>
                  {attachments.map((att: TaskAttachment, idx: number) => {
                    const isImg = att.file_type === 'image' || att.file_url?.match(/\.(jpeg|jpg|png|webp|gif)/i)
                    return (
                      <Pressable
                        key={idx}
                        onPress={() => {
                          if (isImg) {
                            setSelectedImageForLightbox(att.file_url)
                      ...
                      >
                        ...
                        <View style={styles.attachmentInfo}>
                          ...
                          <Text style={styles.attachmentType}>
                            {isImg ? 'Imagen • Toca para ampliar' : 'Documento • Toca para abrir'}
                          </Text>
                        </View>
        ...
        {selectedImageForLightbox && (
          <Modal visible={true} transparent={true} animationType="fade">
            <Pressable
              ...
              onPress={() => setSelectedImageForLightbox(null)}
            >
              <Image
                source={{ uri: selectedImageForLightbox }}
